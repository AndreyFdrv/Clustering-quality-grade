...
{
    class FuzzyAdjustedMutualInformation
    {
        private ArrayList ClusterInfo, ClassInfo;
        public FuzzyAdjustedMutualInformation(ArrayList ClusterInfo, ArrayList ClassInfo)
        {
        ...
        private double Probability(int class_number, int cluster_number)
        {
            ...
            {
                if ((((ArrayList)ClassInfo[i]).Contains(class_number)) && (((ArrayList)ClusterInfo[i]).Contains(cluster_number)))
                    sum++;
            }
        ...
        private double ClassProbability(int class_number)
        {
            ...
            for (int i = 0; i < ClassInfo.Count; i++)
            {
                if (((ArrayList)ClassInfo[i]).Contains(class_number))
                    sum++;
            }
            return (double)sum / ClassInfo.Count;
        }
        private double ClusterProbability(int cluster_number)
        {
            ...
            {
                if (((ArrayList)ClusterInfo[i]).Contains(cluster_number))
                    sum++;
            }
